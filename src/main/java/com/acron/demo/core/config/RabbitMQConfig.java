...
public class RabbitMQConfig {
    ...
    public Queue orderDeadQueue(){
        ...
        args.put("x-dead-letter-exchange", env.getProperty("order.dead.exchange.name"));
        args.put("x-dead-letter-routing-key", env.getProperty("order.dead.routing.key.name"));
        args.put("x-message-ttl", 60000);
        return new Queue(env.getProperty("order.dead.queue.name"),false,false,true,args);
    }

    //死信队列：面向生产端
    @Bean
    public TopicExchange orderDeadExchange(){
        return new TopicExchange(env.getProperty("order.dead.produce.exchange.name"),false,true);
    }

    @Bean
    public Binding orderDeadBinding(){
        return BindingBuilder.bind(orderDeadQueue()).to(orderDeadExchange()).with(env.getProperty("order.dead.produce.routing.key.name"));
    }

    //消费队列
    @Bean
    public Queue orderRealQueue(){
        return new Queue(env.getProperty("order.dead.real.queue.name"),false);
    }

    @Bean
    public TopicExchange orderRealExchange(){
        return new TopicExchange(env.getProperty("order.dead.exchange.name"),false,true);
    }

    @Bean
    public Binding orderRealBinding(){
        return BindingBuilder.bind(orderRealQueue()).to(orderRealExchange()).with(env.getProperty("order.dead.routing.key.name"));
    }
}
